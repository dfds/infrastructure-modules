...
if [ "$ACTION" = "apply-cluster" ]; then
	...
	# Apply the configuration
	terragrunt run-all apply --terragrunt-working-dir "$WORKDIR" --terragrunt-exclude-dir "services" --terragrunt-source-update --terragrunt-non-interactive -input=false -auto-approve
fi
...
if [ "$ACTION" = "temp-hack" ]; then
	...
	kubectl delete ds -n monitoring monitoring-prometheus-node-exporter
fi
