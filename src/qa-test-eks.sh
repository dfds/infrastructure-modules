...
extra_cleanup () {
    ...
    aws --region "$REGION" ec2 describe-network-interfaces --filters "Name=group-name,Values=eks-${CLUSTERNAME}-node" --query "NetworkInterfaces[].NetworkInterfaceId" --output text | xargs -L1 aws --region "$REGION" ec2 delete-network-interface --network-interface-id || true
}
