...
module "traefik_nlb" {
  ...
  albs            = module.traefik_alb_anon.alb_arn_suffix  # ALBs as a list from traefik outputs
...
module "harbor_postgres" {
  ...
  deploy                                 = var.harbor_deploy
  vpc_id                                 = data.terraform_remote_state.cluster.outputs.eks_cluster_vpc_id
  allow_connections_from_security_groups = [data.terraform_remote_state.cluster.outputs.eks_cluster_nodes_sg_id]
  subnet_ids                             = data.terraform_remote_state.cluster.outputs.eks_cluster_subnet_ids
