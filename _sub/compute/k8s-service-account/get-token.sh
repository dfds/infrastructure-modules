...
eval "$(jq -r '@sh "CLUSTER_NAME=\(.cluster_name) DEFAULT_SECRET_NAME=\(.default_secret_name) KUBECONFIG=\(.kubeconfig_path)"')"
...
token=$(kubectl --kubeconfig $KUBECONFIG -n kube-system get secret $DEFAULT_SECRET_NAME -o json | jq '.data.token' | tr -d '"' | base64 --decode)
...
name=`kubectl --kubeconfig $KUBECONFIG config get-contexts $context | awk '{print $3}' | tail -n 1`

# get endpoint of current context 
# endpoint=`kubectl --kubeconfig ~/.kube/config_$CLUSTER_NAME config view -o jsonpath="{.clusters[?(@.name == \"$name\")].cluster.server}"`
endpoint=`kubectl --kubeconfig $KUBECONFIG config view -o jsonpath="{.clusters[?(@.name == \"$name\")].cluster.server}"`
...
certificate=`kubectl --kubeconfig $KUBECONFIG config view --raw -o jsonpath="{.clusters[?(@.name == \"$name\")].cluster.certificate-authority-data}"`
...
jq -n --arg kubeconfig_json "$kubeconfig_content" '{"kubeconfig_json": $kubeconfig_json}'