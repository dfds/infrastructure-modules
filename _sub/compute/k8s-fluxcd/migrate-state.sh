...
# Actually do the state migration for install manifests
terragrunt state pull \
| jq -r '.resources[] | select(.module == "module.platform_fluxcd[0]") | select(.type == "kubectl_manifest") | select(.name == "install") | .instances[] | "\(.index_key) \(.attributes.api_version) \(.attributes.kind) \(.attributes.namespace) \(.attributes.name)"' | sed -e 's/\(.*\)/\L\1/' \
...
# Actually do the state migration for sync manifests
terragrunt state pull \
| jq -r '.resources[] | select(.module == "module.platform_fluxcd[0]") | select(.type == "kubectl_manifest") | select(.name == "sync") | .instances[] | "\(.index_key) \(.attributes.api_version) \(.attributes.kind) \(.attributes.namespace) \(.attributes.name)"' | sed -e 's/\(.*\)/\L\1/' \
| xargs -n5 sh -c 'echo $0 $1/$2/$3/$4' | sed 's/null\///g' | xargs -n2 sh -c 'terragrunt state mv module.platform_fluxcd[0].kubectl_manifest.sync[\"$0\"] module.platform_fluxcd[0].kubectl_manifest.sync[\"$1\"]'
